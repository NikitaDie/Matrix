...
const Matrix& Matrix::getBiggestMatrixByPairweisÑomparison(const Matrix& matrix) {	//can use only when sizes are equal
...
bool Matrix::operator>(const Matrix& matrix)
{
	...
	if (this->size == matrix.size || *this != matrix) {
		if (&getBiggestMatrixByPairweisÑomparison(matrix) == &matrix) return false;
	}
...
bool Matrix::operator<(const Matrix& matrix)
{
	...
	if (this->size == matrix.size || *this != matrix) {
		if (&getBiggestMatrixByPairweisÑomparison(matrix) == &matrix) return true;
	}
